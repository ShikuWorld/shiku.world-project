...
import { create_resource_manager } from "./resources/create_resource_manager";
        ...
        .with(
          ...
          ([module_name, instance_id, game_system_event]) => {
            match(game_system_event)
              .with(
                { SetCamera: P.select() },
                ([entity_id, camera_settings]) => {
                  camera.set_camera_ref(entity_id, module_name);
                  camera.set_camera_settings(camera_settings[2], renderer);
                },
              )
                      ...
                      .with(
                        ...
                        (shake_screen_effect) => {
                          new_shaker({
                            ...
                            isBidirectional:
                              shake_screen_effect.is_bidirectional,
